...
import { getNearbyGarages, reportDriverIncident } from "@/lib/roadsideSupport";
...
const DriverFindGarageScreen = ({
  ...
  useEffect(() => {
    ...
    if (!navigator.geolocation) {
      setLocationError(
        "Location is unavailable on this device. Enable GPS to discover nearest garages.",
      );
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      async () => {
        ...
        setNearbyGarages([]);
        setLocationError(
          "Location permission denied or unavailable. Turn on location to see garages near you.",
        );
  ...
  const informPassengerOfIssue = (
    reason: "puncture" | "mechanical",
    garage?: GarageView,
  ) => {
    if (!location) {
      setLocationError(
        "Cannot inform passenger yet because your location is unavailable.",
      );
      return;
    }
    ...
    reportDriverIncident({
      ...
      nearestGarage: garage
        ? {
            name: garage.name,
            phone: garage.phone,
            distanceKm: Number(garage.distanceKm ?? 0),
          }
        : undefined,
    });

    setLocationError(
      `Passenger notified about ${reason === "puncture" ? "tyre puncture" : "mechanical issue"}. They can now open replacement driver flow.`,
    );
    setStatusText(
      "Issue shared with passenger. You can continue to call garage or view directions.",
    );
  };
    ...
    >
      <div className="px-5 pt-10 pb-28">
        ...
        <div className="mb-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-[0_8px_22px_rgba(15,23,42,0.08)]">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-slate-500">
                Get nearby support points in one clean view.
              </p>
            </div>
          ...
          <p className="mt-3 rounded-lg bg-slate-100 px-3 py-2 text-xs font-medium text-slate-600">
            {statusText}
          </p>
          <div className="mt-3 grid grid-cols-2 gap-2">
            <button
              type="button"
              onClick={() =>
                informPassengerOfIssue("mechanical", nearbyGarages[0])
              }
              className="rounded-xl border border-amber-300 bg-amber-50 py-2 text-xs font-bold text-amber-800"
            >
              Inform Passenger: Mechanical
            </button>
            <button
              type="button"
              onClick={() =>
                informPassengerOfIssue("puncture", nearbyGarages[0])
              }
              className="rounded-xl border border-orange-300 bg-orange-50 py-2 text-xs font-bold text-orange-800"
            >
              Inform Passenger: Puncture
            </button>
          </div>
          {locationError && (
          ...
          {nearbyGarages.length === 0 ? (
            ...
            nearbyGarages.map((entry) => (
              ...
              >
                ...
                <button
                  type="button"
                  onClick={() => informPassengerOfIssue("mechanical", entry)}
                  className="mt-2 w-full rounded-xl border border-amber-300 bg-amber-50 py-2 text-xs font-bold text-amber-800"
                >
                  Inform Passenger About Mechanical Issue
                </button>
