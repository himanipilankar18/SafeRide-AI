...
const DriverVerificationScreen = ({ phoneNumber, onVerified }: DriverVerificationScreenProps) => {
  ...
  const hasRegisteredFace = FACE_STAGES.every((stage) => Boolean(capturesByStage[stage])) && Boolean(referenceEmbedding);
        ...
        try {
          ...
          for (let y = 0; y < 96; y += 1) {
            for (let x = 0; x < 96; x += 1) {
              ...
              const iBin = Math.floor(Math.min(0.999, Math.max(0, center)) * 32);
  ...
  const verifyLiveFaceNow = async () => {
    ...
    try {
      ...
      if (similarity < LITE_THRESHOLD) {
        setHasLiveMlVerification(false);
        throw new Error(`Face mismatch. Similarity ${(similarity * 100).toFixed(1)}%. Try again in better lighting.`);
      }

      setHasLiveMlVerification(true);
      stopCamera();
      setMessage(`Strict in-app ML approved (${(similarity * 100).toFixed(1)}%).`);
    } catch (e) {
