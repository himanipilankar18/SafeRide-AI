...
const DriverVerificationScreen = ({ phoneNumber, onVerified }: DriverVerificationScreenProps) => {
      ...
      image.onload = async () => {
        try {
          ...
          const signature = new Float32Array(
            coarse.length + gradHist.length + intensityHist.length + blockMeans.length + blockStd.length
          );
          let cursor = 0;
  ...
  const verifyLiveFaceNow = async () => {
    ...
    try {
      ...
      stopCamera();
      setMessage("Strict in-app ML approved.");
    } catch (e) {
  ...
  return (
    ...
    >
      <div className="mb-4 pr-8">
        <h2 className="text-xl font-extrabold text-foreground">In-App Driver Face Verification</h2>
      ...
      <div className="space-y-3">
        <div className="rounded-2xl border border-border bg-card p-4">
          <p className="text-sm text-foreground font-medium">Driver: {phoneNumber || "driver-demo"}</p>
        </div>
