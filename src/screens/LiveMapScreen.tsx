...
import { AlertTriangle, Phone, Navigation2, MapPin, Activity } from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";
import { useLiveTracking, LiveTrackingEvent, DeviationAlert } from "@/lib/useLiveTracking";
...
const LiveMapScreen = ({ tripId, userType, source, destination, onEmergency, onEndRide }: LiveMapScreenProps) => {
  ...
  const [routePath, setRoutePath] = useState<[number, number][]>([[source.lat, source.lng]]);
  ...
  const { connect, disconnect, send, isConnected } = useLiveTracking(tripId, userType);
    ...
    switch (event.type) {
      case "location_update":
        if (userType === "passenger" && event.location) {
          ...
          setRoutePath((prev) => [...prev, [event.location!.lat, event.location!.lng]].slice(-300));
        }
        break;
      ...
      case "deviation_alert": {
        ...
        setRiskLevel(event.severity === "danger" ? "danger" : event.severity === "warning" ? "warning" : "safe");
        break;
      }
  ...
  const displayLocation = userType === "driver" ? currentLocation : driverLocation;
    ...
    <div className="relative w-full h-screen bg-black flex flex-col">
      ...
      <div className="flex-1 w-full h-full relative">
        ...
        >
          ...
          <Marker position={[displayLocation.lat, displayLocation.lng]} icon={driverMarkerIcon} />

          {/* Route path */}
          {routePath.length > 1 && (
            <Polyline positions={routePath} color={riskLevel === "danger" ? "#dc2626" : "#2563eb"} weight={3} />
          )}
      ...
      <div className="absolute top-0 left-0 right-0 z-20 bg-gradient-to-b from-black/80 to-transparent p-4">
        <div className="flex justify-between items-start">
          ...
          >
            ...
            {riskLevel === "danger" ? "DANGER" : riskLevel === "warning" ? "WARNING" : "SAFE"}
          </div>
        </div>
      </div>
      ...
      <div className="absolute bottom-0 left-0 right-0 z-20 bg-gradient-to-t from-black/95 to-transparent p-4">
        <div className="grid grid-cols-3 gap-4 mb-4">
          ...
          <div className="bg-gray-900/80 rounded-lg p-3">
            ...
            <div className="text-lg font-bold text-white">{distanceToDestination.toFixed(2)} km</div>
          </div>
          ...
          <div className="bg-gray-900/80 rounded-lg p-3">
            ...
            <div className="text-lg font-bold text-white">{(routePath.length * 0.01).toFixed(2)} km</div>
          </div>

          {/* Status */}
          <div className="bg-gray-900/80 rounded-lg p-3">
            <div className="text-xs text-gray-400">Status</div>
            <div className="text-lg font-bold text-white">{isArrived ? "Arrived! 🎉" : "In transit"}</div>
          </div>
        </div>
          ...
          {alerts.map((alert, idx) => (
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <div>
                  <div className="font-bold">{alert.severity === "danger" ? "DEVIATION!" : "Warning"}</div>
                  <div className="text-xs opacity-90">{alert.message}</div>
                  <div className="text-xs opacity-75 mt-1">Risk: {alert.riskScore}%</div>
                </div>
              </div>
            </motion.div>
          ))}
